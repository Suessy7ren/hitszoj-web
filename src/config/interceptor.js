...
import {hasCompleteInfo} from "../api/auth/user";
...
router.beforeEach((to, from, next) => {
   ...
   */
  if (token) {
    if (hasCompleteInfo(store.state.user)) {
      next();
    } else {
      store.dispatch('user/getInfo').then(() => {
        store.dispatch('user/generatePermittedRouteList', store.state.user.role).then(() => {
          /**
           * 此处是next(to.path)而不是next()
           ...
           * 就会导致进入空页面
           * 此处通过next(to.path)可以重新再走一次导航守卫，确保addRoutes已经完成
           */
